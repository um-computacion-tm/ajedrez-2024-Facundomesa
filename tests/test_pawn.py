...
class TestPawn(unittest.TestCase):
    ...
    def test_pawn_moves_white_starting_position(self):
        ...
        self.assertCountEqual(pawn.possible_moves(position, self.board), expected_moves)
    ...
    def test_pawn_moves_with_blocking_piece(self):
        pawn = Pawn("WHITE",self.board)
        self.board.board[5][4] = Pawn("WHITE", self.board)  # Peón blanco bloqueando el camino
        position = (6, 4)  # Posición inicial para peón blanco
    ...
    # Suponiendo que estamos probando con un peón blanco en (1, 2)
        self.board.set_piece(2, 3, Pawn("BLACK", self.board))  # Colocar un peón enemigo para capturar en (2, 3)
        self.board.set_piece(1, 2, Pawn("WHITE", self.board))  # Colocar el peón blanco en (1, 2)
    ...
    # Verificar que el método possible_moves devuelva los movimientos correctos
